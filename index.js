...
const stepThroughCell = (row, column) => {
    ...
    for(let neighbor of neighbors){
        ...
        if (nextRow < 0 || nextRow >= cellsVertical || nextColumn < 0 || nextColumn >= cellsHorizontal) {
            continue; //// means do not anything for current iteration but loop must go on
      
        }
       
        //// if we have visited the neighbor, continiue the next neighbor
    ...
    row.forEach((open, columnIndex) => {
        if(open){
            ...
            const wall = Bodies.rectangle(
                columnIndex * unitLengthX + unitLengthX/2 , rowIndex * unitLengthY + unitLengthY, unitLengthX, 6 , {
                    label: 'wall',
                    isStatic: true,
                    render:{
                        fillStyle: 'red'
...
verticals.forEach((row, rowIndex) => {
    row.forEach((open, columnIndex) => {
        if(open){
            ...
            const wall = Bodies.rectangle(
                columnIndex * unitLengthX + unitLengthX , rowIndex * unitLengthY + unitLengthY/2, 6, unitLengthY, {
                    label: 'wall',
                    isStatic: true,
                    render:{
                        fillStyle: 'red'
...
const goal = Bodies.rectangle(
              width - unitLengthX/2, height - unitLengthY/2, unitLengthX*.7, unitLengthY*.7, 
              {
                label: 'goal',
                isStatic: true,
                render:{
...
//// win condition && when u simply do console.log(event) then u will see that there is nothing in pairs array because then event occur then it will wiped out all the properties if u look in console it will empty array so do forEach
